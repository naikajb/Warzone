...
// initializing the CurrentState pointer and dynamically allocate memory for the CommandProcessor object
GameEngine::GameEngine(): currentState(nullptr) {
    ...
    stateTransitionMap.insert(pair<Command*, GameState*>(new Command("loadmap"), mapLoadedState));
    stateTransitionMap.insert(pair<Command*, GameState*>(new Command("validatemap"), mapValidated));
    stateTransitionMap.insert(pair<Command*, GameState*>(new Command("addplayer"), playersAdded));
    stateTransitionMap.insert(pair<Command*, GameState*>(new Command("gamestart"), assignReinforcement));
    stateTransitionMap.insert(pair<Command*, GameState*>(new Command("replay"), startState));
    // stateTransitionMap.insert(pair<Command*, GameState*>(new Command("quit"), ));
}
...
void GameEngine::processCommand() {
    CommandProcessor* commandProcessor;
    Command* cmd = commandProcessor->getCommand();

    

    if(commandProcessor->validate(cmd, getCurrentState()->getStateName())) {
        stateTransition(cmd);
...
void GameEngine::stateTransition(Command* cmd){
    GameState* newState = stateTransitionMap.find(cmd)->second;
